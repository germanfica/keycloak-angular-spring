...
Composite Roles with backend-client Client Roles: ROLE_USER
```
...
The `realm-admin` role will need `Composite Roles` feature enabled, as we want to associate `backend-client` client roles to it. To do this:
